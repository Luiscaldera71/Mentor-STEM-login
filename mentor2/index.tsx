...
const firebaseConfig = {
  apiKey: "AIzaSyABxagXnJaW0T1gfXq-TVGAhECd6edey8c",
  authDomain: "mentor-stem-proyectos.firebaseapp.com",
  projectId: "mentor-stem-proyectos",
  storageBucket: "mentor-stem-proyectos.firebasestorage.app",
  messagingSenderId: "162366884017",
  appId: "1:162366884017:web:19823ff5ead6a26691ba90"
};
...
// --- MODEL & API CONFIG ---
// This is no longer used on the client-side
...
async function init() {
  ...
  try {
    // Initialize Firebase
    if (firebaseConfig.apiKey.startsWith("AIzaSyA") && firebaseConfig.apiKey.length > 30) {
    ...
    errorContainer.innerHTML = `<p style="color:red; padding:1em;">Error al inicializar Firebase. Asegúrate de que la configuración sea correcta. Consulta la consola para más detalles.</p>`;
    return;
  }
...
/**
 * Calls the secure backend Firebase Cloud Function to interact with the Gemini API.
...
async function callGeminiAPI(prompt: string): Promise<string> {
    ...
    try {
        console.log("Llamando a la función de backend 'callGemini'...");
        const result = await callGemini({ prompt: prompt });
        console.log("Función de backend ha respondido.");
        return result.data.text;
    } catch (error) {
        console.error("Error al llamar a la función de backend:", error);
        const firebaseError = error as any;
        if (firebaseError.code === 'unavailable' || firebaseError.code === 'internal') {
             throw new Error("El servicio de IA no está disponible o falló. Revisa la consola de Firebase Functions para ver los logs de errores.");
        }
...
async function handleFormSubmit(event: Event) {
  ...
  const fullPromptForProposals = `
    ${SYSTEM_INSTRUCTION}

    ---
    Basado en las reglas anteriores, genera propuestas para los siguientes datos:
    ${initialDataForAI}
  `;
  
  try {
    const fullResponse = await callGeminiAPI(fullPromptForProposals);
    parseAndDisplayProposals(fullResponse);
    showView('proposals');

  } catch (error) {
    console.error("Error al obtener propuestas desde el backend:", error);
    const errorMessage = error instanceof Error ? error.message : "Hubo un error al contactar al asistente. Por favor, inténtalo de nuevo.";
...
async function handleProposalSelection(proposalName: string) {
  ...
  if (!currentProjectData) {
      displayError("No se encontraron los datos del proyecto original. Por favor, vuelve a empezar.", detailedPlanSection);
      showView('form');
      return;
  }
  ...
  const fullPromptForPlan = `
    ${SYSTEM_INSTRUCTION}

    ---
    Basado en las reglas anteriores, el usuario proveyó estos datos iniciales:
    - Grado(s): ${currentProjectData.grade}
    - Tema: ${currentProjectData.topic}
    - Recursos: Opción ${currentProjectData.resources} (${resourceOptionFullText.split(':')[0].trim()})
    - Tiempo estimado: ${currentProjectData.time}

    Ahora, el usuario ha elegido la siguiente propuesta: "${proposalName}".

    Genera el plan de proyecto completo y detallado para la propuesta elegida, siguiendo las reglas de la FASE 2 de forma precisa.
  `;

  try {
    const fullPlanMarkdown = await callGeminiAPI(fullPromptForPlan);
    renderDetailedPlan(fullPlanMarkdown.trim(), proposalName);
  } catch (error) {
    console.error("Error al generar el plan desde el backend:", error);
    const errorMessage = error instanceof Error ? error.message : "Hubo un error al generar el plan detallado. Por favor, inténtalo de nuevo.";
    displayError(errorMessage, detailedPlanSection);
    showView('proposals');
  }
}
...
${markdownText}`;
    ...
    try {
        ...
        console.error("Error generating podcast script via backend:", error);
